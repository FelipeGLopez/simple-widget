...
public final class Example {
    ...
    private static final ArrayList<TdApi.Message> newMessages = new ArrayList<TdApi.Message>();
        ...
        public void onResult(TdApi.Object object) {
            switch (object.getConstructor()) {
                ...
                case TdApi.UpdateNewMessage.CONSTRUCTOR:
                    ...
                    TdApi.Message message = (TdApi.Message) updateNewMessage.message;
                    ...
                    String text = (String) formattedText.text;
                    client.send(new TdApi.GetMe(), new Client.ResultHandler() {
                        @Override
                        public void onResult(TdApi.Object object) {
                            switch (object.getConstructor()) {
                                case TdApi.Error.CONSTRUCTOR:
                                    System.err.println("Receive an error for UpdateNewMessage:" + newLine + object);
                                    break;
                                case TdApi.User.CONSTRUCTOR:
                                    int currentUserId = ((TdApi.User) object).id;
                                    if (text != null && message.senderUserId != currentUserId) {
                                        print("")
                                        newMessages.add(message);
                                        for(TdApi.Message msg : newMessages) {
                                            TdApi.MessageText cntt = (TdApi.MessageText) msg.content;
                                            TdApi.FormattedText fText = (TdApi.FormattedText) cntt.text;
                                            String txt = (String) fText.text;
                                            TdApi.Chat chat = chats.get(msg.chatId);
                                            synchronized (chat) {
                                                System.out.println(chat.title + ": " + txt);
                                            }
                                        }
                                        print("")
                                    }
